...
const TRIAGE_META: Record<TriageLevel, { color: ChipColor; accent: string; rank: number }> = {
...
const TRIAGE_LEVEL_LABELS: Record<TriageLevel, { level: string; note: string }> = {
...
const TRIAGE_BUTTON_COLOR: Record<TriageLevel, 'error' | 'warning' | 'success' | 'info'> = {
  Critical: 'error',
  Emergency: 'warning',
  Urgent: 'warning',
  'Semi-Urgent': 'success',
...
const EMERGENCY_PAGES: Array<{
  ...
  { id: 'triage', label: 'Arrivals & Triage', icon: <MonitorHeartIcon fontSize="small" /> },
  { id: 'bed-board', label: 'Bed Board', icon: <BedIcon fontSize="small" /> },
  { id: 'chart', label: 'Case Tracking', icon: <DescriptionIcon fontSize="small" /> },
];
...
const CASE_TRACKING_TABS: Array<{
  ...
  { id: 'vitals', label: 'Vitals', icon: <MonitorHeartIcon fontSize="small" /> },
  { id: 'safety', label: 'Safety & Labs', icon: <ScienceIcon fontSize="small" /> },
  { id: 'timeline', label: 'Timeline', icon: <TimelineIcon fontSize="small" /> },
  { id: 'checklist', label: 'Checklist', icon: <ViewListIcon fontSize="small" /> },
];
...
const INITIAL_PATIENTS: EmergencyPatient[] = [
  {
    ...
    clinicalNotes: 'Suspected ACS. STEMI pathway initiated. Cardiology informed.',
    updatedAt: '08:52',
  },
  {
    id: 'ER-5102',
...
const INITIAL_BEDS: EmergencyBed[] = [
  ...
  { id: 'Obs-2', zone: 'Observation', status: 'Occupied', patientId: 'ER-5104' },
...
const ORDER_TEMPLATES: Record<OrderType, string[]> = {
  ...
  Medication: ['IV NS 500 mL', 'Ondansetron 4mg IV', 'Nebulization', 'Paracetamol 1g IV'],
  Procedures: ['12-lead ECG', 'Trauma primary survey', 'Wound dressing', 'Splinting'],
};
...
function buildDischargeDraft(patient: EmergencyPatient | undefined): DischargeForm {
  if (!patient) {
...
function OrderStatusChip({ status }: { status: OrderStatus }) {
  ...
  return <Chip size="small" label={status} color={color} variant={status === 'Completed' ? 'filled' : 'outlined'} />;
}
...
interface OrdersPanelProps {
  ...
  orderForm: OrderForm;
  onOrderFormChange: <K extends keyof OrderForm>(field: K, value: OrderForm[K]) => void;
  onAddOrder: () => void;
...
function EmergencyOrdersPanel({
  ...
  return (
    ...
    >
      {!selectedPatient ? (
        <Alert severity="info">Select a patient to place emergency orders.</Alert>
      ) : (
        <Stack spacing={1.25}>
          <Grid container spacing={1}>
            ...
            <Grid xs={12} md={3}>
              <TextField
                ...
                onChange={(event) =>
                  onOrderFormChange('priority', event.target.value as OrderPriority)
                }
                fullWidth
              >
            ...
            <Grid xs={12} md={4}>
              <TextField
                ...
                onChange={(event) => onOrderFormChange('item', event.target.value)}
                fullWidth
                ...
                {rows.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6}>
                      <Typography variant="body2" color="text.secondary" sx={{ py: 1 }}>
                        No emergency orders placed yet.
                      </Typography>
                    </TableCell>
                  </TableRow>
                ) : (
                  rows.map((order) => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell sx={{ minWidth: 180 }}>{order.item}</TableCell>
                      <TableCell>
                        <Chip
                          size="small"
                          ...
                          variant={order.priority === 'Routine' ? 'outlined' : 'filled'}
                        />
                      </TableCell>
                      <TableCell>
...
function ObservationTimelinePanel({
  ...
  return (
    <WorkflowSectionCard
      ...
      action={
        <Button size="small" variant="contained" onClick={onRecordVitals} disabled={!selectedPatient}>
          Record Vitals
        </Button>
      }
    >
      {!selectedPatient ? (
        <Alert severity="info">Select a patient to review the observation timeline.</Alert>
      ) : rows.length === 0 ? (
        <Alert severity="info">No observation entries yet for this patient.</Alert>
      ) : (
        <Stack spacing={1}>
          {rows.map((entry) => (
            <Card
            ...
            >
              <Stack spacing={0.5}>
                ...
                <Typography variant="body2" color="text.secondary">
                  HR {entry.heartRate} bpm · BP {entry.bloodPressure} · Temp {entry.temperature}°C · RR {entry.respiratoryRate}/min · SpO2 {entry.spo2}% · Pain {entry.painScore}/10 · GCS {entry.gcs}
                </Typography>
...
export default function AsapEmergencyPage() {
  ...
  const [activePage, setActivePage] = React.useState<EmergencyPageId>('dashboard');

  const [patients, setPatients] = React.useState<EmergencyPatient[]>(INITIAL_PATIENTS);
  ...
  const [observationLog, setObservationLog] = React.useState<ObservationEntry[]>(
    INITIAL_OBSERVATION_LOG
  );

  const [selectedPatientId, setSelectedPatientId] = React.useState('');
  const [queueFilter, setQueueFilter] = React.useState<'ALL' | TriageLevel>('ALL');
  const [queueSearch, setQueueSearch] = React.useState('');
  const [queueView, setQueueView] = React.useState<QueueViewMode>('table');
  const [queueStatusFilter, setQueueStatusFilter] = React.useState<'ALL' | PatientStatus>('ALL');
  const [queueArrivalFilter, setQueueArrivalFilter] = React.useState<'ALL' | ArrivalMode>('ALL');
  const [queueDoctorFilter, setQueueDoctorFilter] = React.useState<'ALL' | string>('ALL');
  const [caseTrackingTab, setCaseTrackingTab] = React.useState<CaseTrackingTabId>('vitals');
  const [caseTrackingSearch, setCaseTrackingSearch] = React.useState('');
  const [caseTrackingFilter, setCaseTrackingFilter] = React.useState<CaseTrackingSidebarFilter>('All');
  ...
  const [triageForm, setTriageForm] = React.useState<TriageForm>(DEFAULT_TRIAGE);
  const [vitalsForm, setVitalsForm] = React.useState<VitalsForm>(DEFAULT_VITALS_FORM);

  const [orderForm, setOrderForm] = React.useState<OrderForm>(DEFAULT_ORDER_FORM);
  const [dischargeForm, setDischargeForm] = React.useState<DischargeForm>(DEFAULT_DISCHARGE_FORM);
  ...
  const notify = React.useCallback((message: string, severity: ToastSeverity = 'info') => {
  ...
  React.useEffect(() => {
    ...
    const triagePatient = patients.find((entry) => entry.id === triageForm.patientId);
  ...
  const dashboardMetrics = React.useMemo(() => {
    ...
    const bedOccupancyPercent =
      beds.length > 0 ? Math.round((occupiedOrCritical / beds.length) * 100) : 0;
    const avgWaitMinutes =
      patients.length > 0
        ? Math.round(
            patients.reduce((acc, patient) => acc + patient.waitingMinutes, 0) / patients.length
          )
        : 0;
  ...
  const queueKanbanColumns = React.useMemo(
    () =>
      (Object.keys(TRIAGE_META) as TriageLevel[]).map((level) => ({
        level,
        rows: sortedQueueRows.filter((patient) => patient.triageLevel === level),
      })),
    [sortedQueueRows]
  );
  ...
  React.useEffect(() => {
    ...
    if (caseTrackingRows.some((patient) => patient.id === selectedPatientId)) return;
  ...
  const queueDoctorOptions = React.useMemo(
    () => Array.from(new Set(patients.map((patient) => patient.assignedDoctor))).sort(),
    [patients]
  );
  ...
  const filteredBedRows = React.useMemo(
    () => beds.filter((bed) => (bedFilter === 'ALL' ? true : bed.status === bedFilter)),
    [bedFilter, beds]
  );
  ...
  const openTriageAssessment = React.useCallback(
    (patientId: string) => {
      const triagePatient = patients.find((patient) => patient.id === patientId);
      if (!triagePatient) return;
  ...
  const handleSaveTriage = React.useCallback(() => {
    const triagePatient = patients.find((patient) => patient.id === triageForm.patientId);
    if (!triagePatient) {
  ...
  }, [assignBed, assignBedForm.bedId, assignBedForm.patientId, handleOpenPatientChart, notify]);
  ...
  const handleSetBedFree = React.useCallback(
    (bedId: string) => {
      setBeds((prev) =>
        prev.map((bed) =>
          bed.id === bedId
            ? { ...bed, status: 'Free', patientId: null }
            : bed
        )
      );
  ...
  const handleSaveVitals = React.useCallback(() => {
    const targetPatient = patients.find((patient) => patient.id === vitalsForm.patientId);
    if (!targetPatient) {
      notify('Select a patient before recording vitals.', 'warning');
      return;
    }
  ...
  const handleDisposition = React.useCallback(
    (action: 'discharge' | 'admit' | 'transfer') => {
      ...
      setOrders((prev) => prev.filter((order) => order.patientId !== patientId));
    ...
    [dischargeForm.diagnosis, dischargeForm.instructions, notify, openRoute, selectedPatient]
  );
    ...
    return (
      <Grid container spacing={2}>
        ...
        <Grid xs={12} lg={12}>
          <Stack spacing={2}>
            <WorkflowSectionCard
              title="Arrivals & Triage Preview"
              subtitle="Global emergency queue for triage and patient selection"
              action={
                <Stack direction="row" spacing={0.75}>
                  ...
                  <Button size="small" variant="outlined" onClick={() => setActivePage('triage')}>
                    View All
                  </Button>
                </Stack>
              }
              ...
              {sortedQueueRows.length === 0 ? (
                ...
                <TableContainer sx={{ overflowX: 'auto' }}>
                  <Table size="small">
                    ...
                    <TableBody>
                      {sortedQueueRows.slice(0, 8).map((patient) => (
                        <TableRow key={patient.id}>
                          ...
                          <TableCell sx={{ minWidth: 170 }}>{patient.chiefComplaint}</TableCell>
                          <TableCell>
                            <Typography
                              variant="body2"
                              sx={{
                                fontWeight: 700,
                                color:
                                  patient.waitingMinutes > 45
                                    ? 'error.main'
                                    : patient.waitingMinutes > 25
                                      ? 'warning.main'
                                      : 'success.main',
                              }}
                            >
                          ...
                          <TableCell>{patient.assignedBed ?? 'NA'}</TableCell>
        ...
        <Grid container spacing={1}>
          ...
          <Grid xs={12} md={4}>
            <TextField
              ...
              onChange={(event) =>
                handleRegistrationField('arrivalMode', event.target.value as ArrivalMode)
              }
              fullWidth
            >
  ...
  const renderAssignBedModalContent = () => {
    if (!assignBedPatient) {
      return <Alert severity="info">Select a patient from Arrivals & Triage to assign bed.</Alert>;
    }
        ...
        <Grid xs={12} lg={8}>
          <Stack spacing={1.25}>
            ...
            >
              <Stack direction="row" spacing={1.1} alignItems="flex-start">
                ...
                <Box sx={{ minWidth: 0 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {assignBedPatient.id} · {assignBedPatient.mrn} · {assignBedPatient.age} / {assignBedPatient.gender}
                  </Typography>
            ...
            {assignBedZones.map((zoneEntry) => {
              const freeCount = zoneEntry.beds.filter((bed) => bed.status === 'Free').length;
              return (
                        ...
                        >
                          <Stack spacing={0.45}>
                            ...
                            <Typography variant="caption" color="text.secondary" noWrap>
                              {occupiedPatient ? occupiedPatient.name : 'Available'}
                            </Typography>
                          </Stack>
                        </Card>
                      );
          ...
          <Stack spacing={1}>
            <Card
              ...
              sx={{
                ...
                borderRadius: 1.6,
                border: '1px solid',
                borderColor: assignBedForm.bedId ? alpha(theme.palette.primary.main, 0.45) : 'divider',
            ...
            >
              ...
              <Typography variant="caption" color="text.secondary">
                {selectedBed ? `${selectedBed.zone} · ${selectedBed.status}` : 'Choose an available bed from the left panel.'}
              </Typography>
            </Card>
            ...
            <TextField
              ...
              onChange={(event) => handleAssignBedField('physician', event.target.value)}
              fullWidth
            >
            ...
            <TextField
              ...
              onChange={(event) => handleAssignBedField('nurse', event.target.value)}
              fullWidth
            >
            ...
            <TextField
              ...
              onChange={(event) =>
                handleAssignBedField('priority', event.target.value as BedAssignPriority)
              }
              fullWidth
            >
              {(['Immediate', 'High', 'Medium', 'Standard'] as BedAssignPriority[]).map((item) => (
            ...
            <TextField
              ...
              onChange={(event) => handleAssignBedField('notes', event.target.value)}
              multiline
              minRows={3}
  ...
  const renderQueue = () => (
    <WorkflowSectionCard
      title="Arrivals & Triage"
      subtitle="Global emergency queue for triage, reassessment, and selecting an emergency case"
      action={(
        <Button
          size="small"
          variant="contained"
      ...
      <Stack spacing={1.25}>
        <Grid container spacing={1}>
            ...
            <Grid container spacing={1}>
              <Grid xs={12} sm={4}>
                ...
                >
                  ...
                  {(
                    ['Waiting', 'In Treatment', 'Observation', 'Ready for Disposition'] as PatientStatus[]
                  ).map((status) => (
              ...
              <Grid xs={12} sm={4}>
                ...
                >
                  ...
                  {(['Ambulance', 'Walk-in', 'Referral', 'Police'] as ArrivalMode[]).map((mode) => (
          ...
          <Grid xs={12}>
            ...
            >
              <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap>
                {(['ALL', ...Object.keys(TRIAGE_META)] as Array<'ALL' | TriageLevel>).map((level) => (
                  <Chip
                    key={level}
              ...
              <Stack direction="row" spacing={0.75}>
                <Chip
                  label="Table View"
                  clickable
                  color={queueView === 'table' ? 'primary' : 'default'}
                  variant={queueView === 'table' ? 'filled' : 'outlined'}
                  onClick={() => setQueueView('table')}
                />
                <Chip
                  label="Kanban View"
                  clickable
                  color={queueView === 'kanban' ? 'primary' : 'default'}
                  variant={queueView === 'kanban' ? 'filled' : 'outlined'}
        ...
        {queueView === 'table' ? (
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>Assigned Bed</TableCell>
              ...
              <TableBody>
                {sortedQueueRows.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6}>
                      <Stack spacing={1} alignItems="flex-start" sx={{ py: 1 }}>
                        <Typography variant="body2" color="text.secondary">
                          No queue rows found for the selected filter.
                        </Typography>
                        <Button
                          size="small"
                          variant="contained"
                  ...
                  sortedQueueRows.map((patient) => (
                    <TableRow
                      key={patient.id}
                      sx={{
                        borderLeft: '3px solid',
                        borderLeftColor: TRIAGE_META[patient.triageLevel].accent,
                      }}
                    >
                      <TableCell>
                        <Typography variant="body2" sx={{ fontWeight: 700 }}>
                      ...
                      <TableCell>{patient.waitingMinutes} min</TableCell>
                      <TableCell>{patient.assignedBed ?? 'Not Assigned'}</TableCell>
                      <TableCell>{patient.assignedDoctor}</TableCell>
                      <TableCell align="right">
                        <Stack direction={{ xs: 'column', md: 'row' }} spacing={0.75} justifyContent="flex-end">
                          <Button
                            size="small"
                            variant="outlined"
                            onClick={() => openTriageAssessment(patient.id)}
                          >
                            Triage
                          </Button>
          ...
          <Box sx={{ overflowX: 'auto', pb: 0.25 }}>
            <Stack direction="row" spacing={1.25} sx={{ minWidth: 1200 }}>
              {queueKanbanColumns.map((column) => (
                <Card
                  ...
                  sx={{
                    ...
                    p: 1,
                    border: '1px solid',
                    borderColor: alpha(TRIAGE_META[column.level].accent, 0.45),
                    borderRadius: 1.75,
                    bgcolor: alpha(TRIAGE_META[column.level].accent, 0.06),
                    flexShrink: 0,
                  }}
                >
                  <Stack spacing={0.75}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center">
                      ...
                      <Chip
                        ...
                        color={TRIAGE_META[column.level].color}
                        variant="outlined"
                      />
                    </Stack>
                      ...
                      column.rows.map((patient) => (
                        ...
                        >
                          <Stack spacing={0.6}>
                            ...
                            <Typography variant="caption" color="text.secondary">
                              Wait: {patient.waitingMinutes} min · Bed: {patient.assignedBed ?? 'NA'}
                            </Typography>
                            <Typography variant="caption" color="text.secondary">
  ...
  const renderBedBoard = () => (
    ...
    >
      <Stack spacing={1.25}>
        <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap>
          {(['ALL', 'Free', 'Occupied', 'Cleaning', 'Critical'] as BedBoardFilter[]).map(
            (item) => (
              <Chip
                key={item}
    ...
    const createMetricCard = (
      ...
      return (
        <StatTile
          ...
          icon={iconMap[label] ?? <MonitorHeartIcon sx={{ fontSize: 20, color: palette.main }} />}
          sx={{ height: '100%' }}
        />
      );
    };
        ...
        >
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 1.25, mb: 1.5 }}>
            {[
              ...
              {
                label: 'Critical',
                value: patients.filter((patient) => patient.triageLevel === 'Critical').length,
                color: theme.palette.error.main,
              },
              {
                label: 'Ready',
                value: patients.filter((patient) => patient.status === 'Ready for Disposition').length,
            ...
            >
              ...
              >
                ...
                <Typography variant="body2" sx={{ mt: 1, color: alpha(theme.palette.common.white, 0.82) }}>
                  This screen is only for emergency operations: case monitoring, vitals, safety, timeline, and disposition.
                </Typography>
              </Box>

              <Alert severity="info">
                No case selected. Choose a patient from the left queue, or register a new arrival before starting the emergency workflow.
              </Alert>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                ...
                <Button variant="outlined" onClick={() => setRegistrationModalOpen(true)}>
    ...
    const spo2Tone =
      selectedPatient.vitals.spo2 < 90
        ...
        : selectedPatient.vitals.spo2 <= 94
          ? 'warning'
          : 'success';
    const painTone = selectedPatient.vitals.painScore >= 7 ? 'warning' : selectedPatient.vitals.painScore >= 4 ? 'info' : 'success';
    ...
    const checklistColumns = [
      {
        title: 'Airway',
        items: [
          { label: 'Oxygen and saturation monitoring', done: selectedPatient.vitals.spo2 <= 94 || selectedPatientObservations.length > 0 },
          { label: 'Respiratory reassessment', done: selectedPatient.vitals.respiratoryRate > 0 },
          { label: 'Airway backup ready', done: selectedPatient.triageLevel === 'Critical' },
          { label: 'Portable chest support ordered', done: radiologyOrders.some((order) => order.item.toLowerCase().includes('chest')) },
        ],
      },
      {
        title: 'Circulation',
        items: [
          { label: 'BP and pulse trending', done: selectedPatientObservations.length >= 2 },
          { label: 'IV medication / fluid order present', done: selectedPatientOrders.some((order) => order.type === 'Medication') },
          { label: 'Bed assigned', done: Boolean(selectedPatient.assignedBed) },
          { label: 'Doctor assigned', done: Boolean(selectedPatient.assignedDoctor) },
        ],
      },
      {
        ...
        items: [
          ...
          { label: 'At least one result completed', done: selectedPatientOrders.some((order) => order.status === 'Completed') },
          { label: 'Critical flag reviewed', done: selectedPatient.safetyFlags.length > 0 },
        ],
      },
      {
        title: 'Disposition',
        items: [
          ...
          { label: 'Diagnosis captured', done: dischargeForm.diagnosis.trim().length > 0 },
          { label: 'Instructions drafted', done: dischargeForm.instructions.trim().length > 0 },
          { label: 'Ready for disposition', done: selectedPatient.status === 'Ready for Disposition' },
        ],
      },
    ];
                        ...
                        <TableBody>
                          {selectedPatientObservations.length === 0 ? (
                            <TableRow>
                              <TableCell colSpan={7}>
                                <Typography variant="body2" color="text.secondary">
                                  No vitals log available for this case.
                                </Typography>
                              </TableCell>
                            </TableRow>
                          ) : (
                            selectedPatientObservations.slice(0, 6).map((entry) => (
                              <TableRow key={entry.id}>
                                ...
                                <TableCell>{entry.painScore}/10</TableCell>
                          ...
                          <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap sx={{ mt: 0.9 }}>
                            {selectedPatient.allergies.map((allergy) => (
                              <Chip
                                ...
                                label={allergy}
                                sx={{
                                  bgcolor: allergy === 'No known allergies' ? alpha(theme.palette.success.main, 0.1) : alpha(theme.palette.error.main, 0.1),
                                  color: allergy === 'No known allergies' ? 'success.main' : 'error.main',
                                  fontWeight: 700,
                                }}
                              />
                            ))}
                        ...
                        <Grid container spacing={1}>
                          <Grid xs={12} md={3}>
                            <TextField
                              ...
                              label="Type"
                              value={orderForm.type}
                              onChange={(event) => handleOrderField('type', event.target.value as OrderType)}
                              fullWidth
                            >
                              {(['Lab Tests', 'Radiology', 'Medication', 'Procedures'] as OrderType[]).map((type) => (
                          ...
                          <Grid xs={12} md={3}>
                            <TextField
                              ...
                              label="Priority"
                              value={orderForm.priority}
                              onChange={(event) => handleOrderField('priority', event.target.value as OrderPriority)}
                              fullWidth
                            >
                              {(['STAT', 'Urgent', 'Routine'] as OrderPriority[]).map((priority) => (
                                ...
                                <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={1}>
                                  ...
                                  <Stack direction="row" spacing={0.75} alignItems="center" flexWrap="wrap" useFlexGap>
                                    ...
                                    <TextField
                                      ...
                                      sx={{ minWidth: 124 }}
                                    >
                                      {(['Pending', 'In Progress', 'Completed'] as OrderStatus[]).map((status) => (
                                        <MenuItem key={status} value={status}>
                                          {status}
                                        </MenuItem>
                                      ))}
                          ...
                          return (
                            <Stack key={entry.id} direction="row" spacing={1.25}>
                              <Stack alignItems="center" sx={{ width: 28, flexShrink: 0 }}>
                                ...
                                >
                                  {entry.tone === 'critical' ? '!' : entry.tone === 'warning' ? '•' : entry.tone === 'success' ? '✓' : 'i'}
                                </Box>
                ...
                <Stack spacing={1.5}>
                  ...
                  <Card elevation={0} sx={{ borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
                    ...
                    <Stack spacing={1.25} sx={{ p: 1.75 }}>
                      ...
                      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                        <Button variant="outlined" color="success" onClick={() => setDischargePreviewOpen(true)}>
                          Preview AVS
                        </Button>
                        <Button variant="contained" onClick={() => handleDisposition('admit')}>
                          Admit to IPD
                        </Button>
                        <Button variant="outlined" color="success" onClick={() => handleDisposition('discharge')}>
                          Discharge
                        </Button>
                        <Button variant="outlined" color="warning" onClick={() => handleDisposition('transfer')}>
                          Transfer
                        </Button>
                      </Stack>
                    </Stack>
    ...
    >
      ...
      >
        <ModuleHeaderCard
          ...
          chips={[
            { label: `Queue ${dashboardMetrics.totalPatients}`, color: 'primary', variant: 'outlined' },
            { label: `${dashboardMetrics.criticalPatients} Critical`, color: 'error', variant: 'filled' },
            { label: `${dashboardMetrics.availableBeds} Beds Free`, color: 'success', variant: 'outlined' },
            ...(selectedPatient
              ? [{ label: `Case: ${selectedPatient.id}`, color: 'info' as const, variant: 'outlined' as const }]
              : [{ label: 'Case: none selected', color: 'default' as const, variant: 'outlined' as const }]),
          ]}
          actions={
            ...
            <Stack direction="row" spacing={0.65} flexWrap="wrap" useFlexGap>
              {BED_ASSIGN_FLOW_STEPS.map((step, index) => (
                <Chip
                  ...
                  size="small"
                  label={`${index + 1}. ${step}`}
                  color={index < 1 ? 'success' : index === 1 ? 'primary' : 'default'}
      ...
      >
        ...
        >
          ...
          <Button variant="contained" onClick={handleSaveTriage} startIcon={<MonitorHeartIcon />}>
        ...
        <DialogContent sx={{ px: 2.25, py: 2 }}>
          <Stack spacing={1.5}>
            <TextField
              ...
              onChange={(event) => handleVitalsField('patientId', event.target.value)}
              fullWidth
            >
              {patients.map((patient) => (
                <MenuItem key={patient.id} value={patient.id}>
            ...
            <Grid container spacing={1}>
              <Grid xs={12} sm={6} md={4}>
                <TextField
                  ...
                  onChange={(event) => handleVitalsField('heartRate', event.target.value)}
                  fullWidth
                />
              </Grid>
              <Grid xs={12} sm={6} md={4}>
                <TextField
                  ...
                  onChange={(event) => handleVitalsField('bloodPressure', event.target.value)}
                  fullWidth
                />
              </Grid>
              <Grid xs={12} sm={6} md={4}>
                <TextField
                  ...
                  onChange={(event) => handleVitalsField('temperature', event.target.value)}
                  fullWidth
                />
              </Grid>
              <Grid xs={12} sm={6} md={4}>
                <TextField
                  ...
                  onChange={(event) => handleVitalsField('respiratoryRate', event.target.value)}
                  fullWidth
                />
              </Grid>
              <Grid xs={12} sm={6} md={4}>
                <TextField
                  ...
                  onChange={(event) => handleVitalsField('spo2', event.target.value)}
                  fullWidth
                />
              </Grid>
              <Grid xs={12} sm={6} md={2}>
                <TextField
                  ...
                  onChange={(event) => handleVitalsField('painScore', event.target.value)}
                  fullWidth
                />
              </Grid>
              <Grid xs={12} sm={6} md={2}>
      ...
      >
        ...
        <DialogContent sx={{ px: 2.25, py: 2 }}>
          {!selectedPatient ? (
            ...
            <Stack spacing={1.5}>
              <Card elevation={0} sx={{ p: 1.5, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
                <Stack spacing={0.75}>
                  <Typography variant="h6" sx={{ fontWeight: 800 }}>
                    {selectedPatient.name}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    {selectedPatient.id} · {selectedPatient.mrn} · {selectedPatient.age}y / {selectedPatient.gender}
                  </Typography>
                  <Typography variant="body2">
                    <strong>Diagnosis:</strong> {dischargeForm.diagnosis || '--'}
                  </Typography>
                  <Typography variant="body2">
                    <strong>Condition on exit:</strong> {dischargeForm.condition || '--'}
                  </Typography>
                  <Typography variant="body2">
                    <strong>Instructions:</strong> {dischargeForm.instructions || '--'}
                  </Typography>
                  <Typography variant="body2">
                    <strong>Follow-up:</strong> {dischargeForm.followUp || '--'}
                  </Typography>
                  <Typography variant="body2">
                    <strong>Medication advice:</strong> {dischargeForm.medications || '--'}
                  </Typography>
                  <Typography variant="body2">
                    <strong>Destination:</strong> {dischargeForm.destination || '--'}
                  </Typography>
                </Stack>
              </Card>
            </Stack>
        ...
        >
          ...
          <Button
            ...
            onClick={() =>
              selectedPatient
                ? openRoute('/ipd/discharge', { tab: 'avs', mrn: selectedPatient.mrn })
                : undefined
            }
