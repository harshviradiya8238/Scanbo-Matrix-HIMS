...
import { type EmergencyPatient, type EmergencyBed, type BedAssignForm, type BedAssignPriority } from "../../types";
import { BED_ASSIGN_NURSES, BED_ASSIGN_PHYSICIANS, BED_STATUS_META } from "../../AsapEmergencyData";
...
interface AssignBedModalContentProps {
  ...
  beds: EmergencyBed[];
  handleAssignBedField: <K extends keyof BedAssignForm>(field: K, value: BedAssignForm[K]) => void;
...
export function AssignBedModalContent({
  ...
  if (!assignBedPatient) {
    return (
      <Alert severity="info">Select a patient from Arrivals & Triage to assign bed.</Alert>
    );
  }
            ...
            return (
              <Box key={zoneEntry.zone}>
                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 0.75 }}>
                  ...
                  <Chip size="small" label={`${freeCount}/${zoneEntry.beds.length} free`} color={freeCount > 0 ? "success" : "warning"} variant="outlined" />
                </Stack>
                    ...
                    return (
                      <Card
                        ...
                        sx={{
                          ...
                          borderRadius: 1.5,
                          border: "1px solid",
                          borderColor: selected ? alpha("primary.main", 0.7) : alpha("divider", 0.75),
                          bgcolor: selected ? alpha("primary.main", 0.08) : "background.paper",
        ...
        <Stack spacing={1}>
          <Card
            ...
            sx={{
              ...
              borderRadius: 1.6,
              border: "1px solid",
              borderColor: assignBedForm.bedId ? alpha("primary.main", 0.45) : "divider",
              bgcolor: assignBedForm.bedId ? alpha("primary.main", 0.05) : alpha("background.default", 0.6),
            }}
          >
          ...
          <TextField
            ...
            onChange={(event) => handleAssignBedField("priority", event.target.value as BedAssignPriority)}
            fullWidth
          >
            {(["Immediate", "High", "Medium", "Standard"] as const).map((item) => (
