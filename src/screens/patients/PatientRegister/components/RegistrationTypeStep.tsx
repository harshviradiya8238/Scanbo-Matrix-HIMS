...
export default function RegistrationTypeStep({
  ...
  const [abhaModalMode, setAbhaModalMode] = React.useState<AbhaMode | null>(
    null,
  );
  ...
  const [aadhaarMethod, setAadhaarMethod] =
    React.useState<AadhaarMethod>("otp");
  ...
  const [abhaLinkedStatus, setAbhaLinkedStatus] =
    React.useState<AbhaProfile | null>(null);
  ...
  const abhaBaseAddress = React.useMemo(() => {
    const preferred = sanitizeAbhaAddress(
      values.abhaAddress || values.patientName || "patient",
    );
  ...
  const applyIdentityPrefill = (payload: {
    ...
    const nextLastName =
      nameParts.slice(1).join(" ") || payload.fallbackLastName;
    ...
    const nextMobileDigits = toDigits(
      payload.mobile || payload.fallbackMobile,
    ).slice(0, 10);
  ...
  const openAbhaFlow = (mode: AbhaMode) => {
    ...
    setLinkAbhaNumber(
      formatAbhaNumber(values.abhaNumber || STATIC_ABHA_NUMBER),
    );
  ...
  const handleRegistrationCountryChange = (
    nextCountry: "india" | "international",
  ) => {
  ...
  const allCreateConsentsAccepted =
    Object.values(createConsents).every(Boolean);

  const handleLinkStepOneNext = () => {
    if (!linkStepOneValid) {
      openToast(
        "Select a method and enter valid details to continue.",
        "warning",
      );
      return;
    }
  ...
  const handleCreateStepOneNext = () => {
    if (!createStepOneValid) {
      openToast(
        "Enter valid details for the selected ABHA creation method.",
        "warning",
      );
      return;
    }
  ...
  const handleCreateAddressNext = () => {
    if (!isValidAbhaAddress(normalizedCreateAddress)) {
      openToast(
        "ABHA address must be 3-18 characters with letters, numbers, dot, or underscore.",
        "warning",
      );
      return;
    }
  ...
  const handleCreateFinalize = () => {
    if (!allCreateConsentsAccepted) {
      openToast(
        "Please accept all consent checkboxes to create ABHA.",
        "warning",
      );
      return;
    }
  ...
  const handleVerifySendOtp = () => {
    if (!isValidAbhaNumber(verifyAbhaNumber)) {
      openToast(
        "Enter a valid 14-digit ABHA number before verification.",
        "warning",
      );
      return;
    }
  ...
  const handleApplyVerifiedAbha = () => {
    ...
    applyAbhaToForm(
      profile,
      "otp_verified",
      "ABHA verification status updated.",
    );
  };
  ...
  const abhaTotalSteps =
    abhaModalMode === "create" ? 5 : abhaModalMode === "link" ? 4 : 3;
  ...
  const renderMethodCard = (
    ...
    >
      ...
      <Typography
        ...
        sx={{ lineHeight: 1.5, mt: 0.35, display: "block" }}
      >
        {description}
      </Typography>
    </Box>
  ...
  const renderAbhaDialogBody = () => {
    ...
    if (abhaModalMode === "link") {
      if (abhaModalStep === 1) {
        return (
          <Stack spacing={1.5}>
            <Alert severity="info">
              Choose a method to link an existing ABHA account.
            </Alert>
            ...
            {abhaMethod === "qr" ? (
              <Alert severity="success">
                QR scan simulated. Next will move directly to profile
                confirmation.
              </Alert>
            ) : null}
          </Stack>
      ...
      if (abhaModalStep === 2) {
        return (
          <Stack spacing={1.4}>
            <Alert severity="success">
              OTP sent. Enter the 6-digit OTP to continue linking ABHA.
            </Alert>
            <Input
              label="OTP"
      ...
      if (abhaModalStep === 3) {
        ...
        return (
          <Stack spacing={1.2}>
            ...
            <Alert severity="info">
              Confirm to link this ABHA with current patient registration.
            </Alert>
          </Stack>
        );
      }
    ...
    if (abhaModalMode === "create") {
        ...
        return (
          <Stack spacing={1.5}>
            ...
            {abhaMethod === "driving" ? (
              <>
                ...
                <Input
                  ...
                  onChange={(event: React.ChangeEvent<HTMLInputElement>) =>
                    setCreateDob(event.target.value)
                  }
                />
              </>
            ) : null}
            ...
            <Alert severity="warning">
              Patient consent is mandatory for ABHA creation and linking.
            </Alert>
          </Stack>
        );
      }
      ...
      if (abhaModalStep === 2) {
        return (
          <Stack spacing={1.4}>
            <Alert severity="success">
              OTP sent. Enter the 6-digit OTP to verify identity.
            </Alert>
            <Input
              label="OTP"
              placeholder="Enter 6-digit OTP"
      ...
      if (abhaModalStep === 3) {
        return (
          <Stack spacing={1.2}>
            <Alert severity="info">
              Set a unique ABHA address for this patient.
            </Alert>
            <Input
            ...
            <Stack direction="row" spacing={0.8} flexWrap="wrap">
              {[
                ...
                <Chip
                  ...
                  size="small"
                  onClick={() =>
                    setCreateAddress(suggestion.replace("@abdm", ""))
                  }
                  sx={{ mt: 0.5 }}
                />
              ))}
      ...
      if (abhaModalStep === 4) {
        return (
          <Stack spacing={1.2}>
            <Alert severity="info">
              Review and accept all consents to create ABHA.
            </Alert>
            <Box
              sx={{
                p: 1.25,
    ...
    if (abhaModalStep === 1) {
      return (
        <Stack spacing={1.5}>
          <Alert severity="info">
            Enter ABHA number and choose OTP verification method.
          </Alert>
          <Input
            label="ABHA Number"
            placeholder="XX-XXXX-XXXX-XXXX"
    ...
    if (abhaModalStep === 2) {
      return (
        <Stack spacing={1.4}>
          <Alert severity="success">
            OTP sent. Enter the OTP to complete ABHA verification.
          </Alert>
          <Input
            label="OTP"
            placeholder="Enter 6-digit OTP"
    ...
    const profile =
      abhaResult ||
      buildProfile({ number: verifyAbhaNumber || STATIC_ABHA_NUMBER });
    ...
    return (
      <Stack spacing={1.2}>
        ...
        <Alert severity="success">
          Verification will update ABHA status to Verified via OTP.
        </Alert>
      </Stack>
    );
  };
  ...
  const renderAbhaDialogActions = () => {
      ...
      if (abhaModalStep === 1) {
        return (
          <>
            ...
            <Button
              type="button"
              onClick={handleLinkStepOneNext}
              variant="contained"
              disabled={!linkStepOneValid}
            >
              Next
            </Button>
          </>
      ...
      if (abhaModalStep === 2) {
        return (
          <>
            ...
            <Button
              type="button"
              onClick={() => setAbhaOtpTimer(OTP_DURATION_SECONDS)}
              variant="text"
              disabled={abhaOtpTimer > 0}
            >
              Resend OTP
            </Button>
            <Button
              type="button"
              onClick={handleLinkOtpVerify}
              variant="contained"
              disabled={!isValidOtp(abhaOtp)}
            >
              Verify OTP
            </Button>
          </>
    ...
    if (abhaModalMode === "create") {
      if (abhaModalStep === 1) {
        return (
          <>
            ...
            <Button
              type="button"
              onClick={handleCreateStepOneNext}
              variant="contained"
              disabled={!createStepOneValid}
            >
              Next
            </Button>
          </>
      ...
      return (
        <>
          ...
          <Button
            type="button"
            onClick={handleApplyCreatedAbha}
            variant="contained"
          >
            Apply to Form
          </Button>
    ...
    return (
      <Button
        type="button"
        onClick={handleApplyVerifiedAbha}
        variant="contained"
        fullWidth
      >
        Apply to Form
  ...
  return (
    <Stack spacing={2}>
        ...
        >
          ...
          >
            ...
            <Stack direction="row" spacing={0.75} flexWrap="wrap">
              ...
              <Chip
                size="small"
                label={isIndia ? "ABHA / Aadhaar" : "Passport / Visa"}
                sx={chipSx}
              />
            </Stack>
          </Stack>
        ...
        <Box sx={{ p: 2.25 }}>
          {isIndia ? (
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              Passport is the primary identity for international registration.
              Capture visa and country-of-residence details before submission.
            </Alert>
          )}
            ...
            {isIndia ? (
              <>
                      ...
                      >
                        ...
                        <Box sx={{ minWidth: 0, flex: 1 }}>
                          ...
                          <Typography
                            ...
                            sx={{ display: "block" }}
                          >
                            {abhaLinkedStatus.number} ·{" "}
                            {abhaLinkedStatus.address}
                          </Typography>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                            sx={{ display: "block" }}
                          >
                            {abhaLinkedStatus.name} ·{" "}
                            {abhaLinkedStatus.verifiedVia}
                          </Typography>
                        </Box>
                        <Button
                          type="button"
                ...
                <Grid xs={12} md={4}>
                  <FormSelect
                    name="bplStatus"
                    label="BPL / APL Status"
                    options={bplStatusOptions}
                  />
                </Grid>
              </>
            ) : (
              <>
                ...
                <Grid xs={12} md={4}>
                  <FormDatePicker
                    ...
                    label="Visa Validity Date"
                  />
                </Grid>
                <Grid xs={12} md={4}>
                  <FormDatePicker
                    name="arrivalInIndiaDate"
                    label="Date of Arrival in India"
                  />
                </Grid>
                <Grid xs={12} md={4}>
                  <FormTextField
                    name="intlInsuranceProvider"
                    label="International Insurance / TPA"
                  />
                </Grid>
                <Grid xs={12} md={4}>
                  <FormTextField
                    name="intlPolicyMemberId"
                ...
                <Grid xs={12} md={4}>
                  <FormTextField
                    name="embassyContact"
                    label="Embassy / Consulate Contact"
                  />
                </Grid>
              </>
            )}
      ...
      <Dialog
        open={Boolean(abhaModalMode)}
        onClose={closeAbhaFlow}
        fullWidth
        maxWidth="sm"
      >
        <DialogTitle
        ...
        <DialogActions sx={{ px: 2.2, py: 1.3 }}>
          {renderAbhaDialogActions()}
        </DialogActions>
      </Dialog>

      <Dialog
        open={aadhaarDialogOpen}
        onClose={closeAadhaarDialog}
        fullWidth
        maxWidth="xs"
      >
        <DialogTitle
        ...
        <DialogContent dividers>
          {aadhaarDialogStep === 1 ? (
            <Stack spacing={1.35} sx={{ mt: 0.4 }}>
              <Alert severity="info">
                Full Aadhaar is never stored. Only masked value is retained in
                form.
              </Alert>
              <Input
                label="Aadhaar Number"
            ...
            <Stack spacing={1.35} sx={{ mt: 0.4 }}>
              <Alert severity="success">
                OTP sent to Aadhaar-linked mobile. Enter OTP to verify.
              </Alert>
              <Input
                label="OTP"
                placeholder="Enter 6-digit OTP"
                ...
                onChange={(event: React.ChangeEvent<HTMLInputElement>) =>
                  setAadhaarOtp(
                    toDigits(event.target.value).slice(0, OTP_LENGTH),
                  )
                }
              />
              <Typography variant="caption" color="text.secondary">
        ...
        <DialogActions sx={{ px: 2.2, py: 1.3 }}>
          {aadhaarDialogStep === 1 ? (
            ...
            <>
              <Button
                type="button"
                onClick={() => setAadhaarDialogStep(1)}
                variant="outlined"
              >
                Back
              </Button>
              ...
              <Button
                ...
                variant="contained"
                disabled={!isValidOtp(aadhaarOtp)}
              >
                Verify
              </Button>
            </>
