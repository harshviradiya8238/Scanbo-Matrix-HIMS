...
export default function OpdCalendarPage() {
  ...
  const handleCreateBooking = (sendToQueue: boolean) => {
    ...
    const created: OpdAppointment = {
      ...
      status: sendToQueue ? "Checked-In" : "Pending",
  ...
  const handleQuickCheckIn = React.useCallback(
    (appt: OpdAppointment) => {
      ...
      dispatch(
        addEncounter({
          ...
          triageNote: "Checked in from calendar popover.",
          allergies: ["No known allergies"],
          problems: [],
          notes: [],
          orders: [],
      ...
      setSnackbar({
        open: true,
        message: `${appt.patientName} checked in and sent to queue.`,
        severity: "success",
      });
      setSelectedEvent(null);
      setEventAnchor(null);
    ...
    [dispatch, guardCalendarAction, router, withMrn],
  );
                ...
                <Chip
                  color={
                    selectedEvent.status === "Pending"
                      ? "primary"
                      : appointmentStatusColor[selectedEvent.status] ||
                        "default"
                  }
              ...
              <Stack direction="row" spacing={1}>
                ...
                {selectedEvent.status === "Pending" && (
                  <Button
                    ...
                    variant="contained"
                    disabled={!canManageCalendar}
                    onClick={() => handleQuickCheckIn(selectedEvent)}
                  >
                    Check In
                  </Button>
                )}
