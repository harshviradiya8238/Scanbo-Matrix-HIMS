...
function ClientDetailView({
  ...
  const pending = samples.filter(
    (s) => s.status !== "published" && s.status !== "verified",
  ).length;
...
export default function LabClientsPage() {
  ...
  const sampleCount = (clientId: string) =>
    samples.filter((s) => s.client === clientId).length;

  const clientColumns = React.useMemo<CommonColumn<LabClient>[]>(
    () => [
      {
        headerName: "ID",
        field: "id",
      ...
      },
      {
        ...
        renderCell: (row) => (
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              router.push(`/lab/clients?id=${row.id}`);
            }}
          >
            Manage
          </Button>
  ...
  return (
    ...
    >
      ...
      >
        <Stack spacing={2}>
          <CommonDataGrid<LabClient>
            rows={filtered}
            columns={clientColumns}
            getRowId={(row) => row.id}
            searchPlaceholder="Search clients..."
            onRowClick={(row) => router.push(`/lab/clients?id=${row.id}`)}
          />
        </Stack>
      </LabWorkspaceCard>
      <AddClientModal
