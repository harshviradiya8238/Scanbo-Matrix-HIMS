...
function statusChip(status: InventoryItemStatus) {
  ...
  if (status === "low")
    return <Chip size="small" label="Low Stock" color="warning" />;
      ...
      >
        ...
        <Card elevation={0} sx={{ borderRadius: 1.5, p: 2 }}>
          ...
          <Typography
            ...
            sx={{
              fontWeight: 700,
              color: thresholdGap < 0 ? "error.main" : "success.main",
            }}
          >
      ...
      >
        ...
        <Card elevation={0} sx={{ borderRadius: 1.5, p: 2 }}>
          ...
          <Stack spacing={2}>
            ...
            <Typography variant="caption" color="text.secondary">
              Use this panel to keep inventory levels accurate after receipt or
              test run usage.
            </Typography>
          </Stack>
        </Card>
...
export default function LabInventoryPage() {
  ...
  const inventoryColumns = React.useMemo<CommonColumn<LabInventoryItem>[]>(
    () => [
      {
        headerName: "SKU",
        field: "sku",
      ...
      },
      {
        ...
        field: "id",
        width: 130,
        renderCell: (row) => statusChip(getStatus(row)),
      },
      {
        headerName: "Actions",
        field: "id",
        ...
        renderCell: (row) => (
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              router.push(`/lab/inventory?id=${row.id}`);
            }}
          >
            Manage
          </Button>
        ...
        <LabWorkspaceCard current="inventory">
          <Stack spacing={2}>
            <InventoryDetailView
              ...
              onAdjust={(mode, quantity) =>
                handleAdjust(selectedItem, mode, quantity)
              }
            />
          </Stack>
        </LabWorkspaceCard>
    ...
    >
      ...
      >
        <Stack spacing={2}>
          ...
          <CommonDataGrid<LabInventoryItem>
            rows={rows}
            columns={inventoryColumns}
            getRowId={(row) => row.id}
            searchPlaceholder="Search items..."
            onRowClick={(row) => router.push(`/lab/inventory?id=${row.id}`)}
          />
        </Stack>
      </LabWorkspaceCard>
